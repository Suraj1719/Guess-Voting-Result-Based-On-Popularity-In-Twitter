...
public_tweets = api.search('BJD')
bjd = 0
y = 0
for tweet in public_tweets:
    analysis = TextBlob(tweet.text)
    aap = analysis.sentiment.polarity + bjd
    y = analysis.sentiment.subjectivity + y
print("BJD")
print(f"Polarity of all tweets is : {bjd}")
print(f"Subjectivity of all tweets is : {y}")
print("")
...
height = [bjp, congress, bjd, shiv_sena, npp]  # list of parties

tick_label = ['BJP', 'CONGRESS', 'BJD', 'SHIV SENA', 'NPP']  # title label
...
public_tweets = api.search('Naveen Pattnayak')
h = 0
y = 0
for tweet in public_tweets:
    analysis = TextBlob(tweet.text)
...
tick_label = ['Narendra Modi', 'Rahul Gandhi', 'Naveen Pattnayak', 'Lalu Prashad Yadav', 'Akhilesh yadav']
